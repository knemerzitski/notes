...
import { AnyEntry, TailDocumentHistory } from './tail-document-history';

import { Changeset } from '../changeset/changeset';

describe('TailDocumentHistory', () => {
  ...
  describe('unshift', () => {
    it.each([
      ...
      const history = new TailDocumentHistory({
      ...
      expect(history.tailDocument.toString()).toStrictEqual(
      ...
      expect(history.getHeadDocument().toString()).toStrictEqual(
  ...
  describe('composeOnAllEntries', () => {
    let history: TailDocumentHistory;

    beforeEach(() => {
      history = new TailDocumentHistory();
      history.push([
        createEntry(['cd']),
        createEntry(['ab', [0, 1]]),
        createEntry([[0, 1], '|', [2, 3]]),
    ...
    it('inserts at start on middle target index', () => {
      ...
      expect(history.tailDocument.toString()).toStrictEqual('(0 -> 11)["[before cd]"]');
      expect(history.entries.map((e) => e.execute.changeset.toString())).toStrictEqual([
        '(11 -> 13)[0 - 10, "cd"]',
        '(13 -> 15)[0 - 10, "ab", 11 - 12]',
        '(15 -> 16)[0 - 12, "|", 13 - 14]',
    ...
    it('deletes middle on middle target index', () => {
      ...
      expect(history.tailDocument.toString()).toStrictEqual('(0 -> 8)["[del ij]"]');
      expect(history.entries.map((e) => e.execute.changeset.toString())).toStrictEqual([
        '(8 -> 10)[0 - 7, "cd"]',
        '(10 -> 12)[0 - 7, "ab", 8 - 9]',
        '(12 -> 13)[0 - 9, "|", 10 - 11]',
    ...
    it('removes an entry that is no longer meaningful', () => {
      ...
      expect(history.tailDocument.toString()).toStrictEqual('(0 -> 0)[]');
      expect(history.entries.map((e) => e.execute.changeset.toString())).toStrictEqual([
        '(0 -> 2)["cd"]',
        '(2 -> 3)["|", 0 - 1]',
        '(3 -> 13)[0 - 2, "efghijklmn"]',
    ...
    it('removes all older entries, keeping only what was already removed', () => {
      ...
      expect(history.getHeadDocument().toString()).toStrictEqual('(0 -> 2)["__"]');
    });

    it('replaces tail document', () => {
      history.composeOnAllEntries(
