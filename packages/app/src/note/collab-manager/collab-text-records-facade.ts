...
export class CollabTextRecordsFacade implements CollabServiceServerFacade {
  ...
  constructor(
    ...
    readonly cache: Pick<
      ApolloCache<unknown>,
      'watchFragment' | 'readFragment' | 'identify'
    >
  ) {
  ...
  updateCollabTextId(collabTextId: string) {
    ...
    const headObservable = this.cache.watchFragment({
    ...
    const recordsObservable = this.cache.watchFragment({
