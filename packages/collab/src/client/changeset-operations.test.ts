...
describe('insertionOperation', () => {
  it.each([
    ...
    ({
      ...
      const { changeset, inverseChangeset, newSelectionPos } = insertionOperation(
        insertText,
        textValue,
        selection
      );
      ...
      expect(newSelectionPos).toStrictEqual(expectedSelectionPos);
...
describe('deletionCountOperation', () => {
  it.each([
    ...
    ({
      ...
      const delOp = deletionCountOperation(deleteCount, textValue, selection);
      assert(delOp != null);
      const { changeset, inverseChangeset, newSelectionPos } = delOp;
